...
而映射就需要存储映射表。在现代CPU架构中，映射关系通常被存储在物理内存上一个被称之为页表(page table)的地方。如下图：
...
为了进一步优化性能，现代CPU架构引入了TLB，用来缓存一部分经常访问的页表内容。如下图：

![avatar](./pictures/win_handler_2.png)
...
![avatar](./pictures/win_handler_3.png)
...
![avatar](./pictures/win_handler_6.png)
...
原创，转载请注明来自http://blog.csdn.net/wenzhou121
